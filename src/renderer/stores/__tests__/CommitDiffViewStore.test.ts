...
describe("CommitDiffViewStore", () => {
  ...
  beforeEach(() => {
    ...
    mockGitService.listCommitFiles = vi.fn()
    mockGitService.getCommitDiff = vi.fn()
    store = new CommitDiffViewStore(workspacePath, mockCommit, mockGitService as never)
  })
  ...
  describe("loadFiles", () => {
    it("should load files from git service", async () => {
      ...
      vi.mocked(mockGitService.listCommitFiles).mockResolvedValue(mockFiles)
      vi.mocked(mockGitService.getCommitDiff).mockResolvedValue("diff content")

      await store.loadFiles()

      expect(mockGitService.listCommitFiles).toHaveBeenCalledWith(workspacePath, "abc1234")
    ...
    it("should auto-select first file after loading", async () => {
      ...
      expect(mockGitService.getCommitDiff).toHaveBeenCalledWith(
        workspacePath,
        "abc1234",
        "src/first.ts",
        "M"
    ...
    it("should not auto-select if no files", async () => {
      vi.mocked(mockGitService.listCommitFiles).mockResolvedValue([])

      await store.loadFiles()

      expect(store.files).toEqual([])
      expect(store.selectedFile.path).toBe("")
      expect(mockGitService.getCommitDiff).not.toHaveBeenCalled()
    })

    it("should handle errors", async () => {
      vi.mocked(mockGitService.listCommitFiles).mockRejectedValue(new Error("Failed to list files"))
    ...
    it("should set filesLoading during fetch", async () => {
      ...
      vi.mocked(mockGitService.listCommitFiles).mockReturnValue(pendingPromise)
  ...
  describe("fetchDiff", () => {
    ...
    it("should fetch diff from git service", async () => {
      ...
      vi.mocked(mockGitService.getCommitDiff).mockResolvedValue(mockDiff)
    ...
    it("should cache diffs", async () => {
      ...
      // First fetch
      await store.fetchDiff(testFile)
      expect(mockGitService.getCommitDiff).toHaveBeenCalledTimes(1)

      // Second fetch - should use cache
      await store.fetchDiff(testFile)
      expect(mockGitService.getCommitDiff).toHaveBeenCalledTimes(1) // Still 1
      expect(store.diff).toBe(mockDiff)
      expect(store.status).toBe("done")
    })
    ...
    it("should use different cache keys for different files", async () => {
      ...
      vi.mocked(mockGitService.getCommitDiff)
      ...
      expect(mockGitService.getCommitDiff).toHaveBeenCalledTimes(2)
    })

    it("should handle errors", async () => {
      vi.mocked(mockGitService.getCommitDiff).mockRejectedValue(new Error("Diff failed"))
    ...
    it("should set loading during fetch", async () => {
      ...
      vi.mocked(mockGitService.getCommitDiff).mockReturnValue(pendingPromise)
  ...
  describe("selectFile", () => {
    it("should update selectedFile and fetch diff", async () => {
      ...
      vi.mocked(mockGitService.getCommitDiff).mockResolvedValue("new diff")
      ...
      await vi.waitFor(() => {
        expect(mockGitService.getCommitDiff).toHaveBeenCalled()
      })
    })
  })
  ...
  describe("reset", () => {
    ...
    it("should clear the cache", async () => {
      ...
      vi.mocked(mockGitService.getCommitDiff).mockResolvedValue("diff")
  ...
  describe("createCommitDiffViewStore factory", () => {
    it("should create a new store instance", () => {
      ...
      const newStore = createCommitDiffViewStore("/another/path", commit, mockGitService as never)
