...
import { X, ChevronDown, ChevronRight, PanelLeftClose, PanelLeft, GitCommit } from "lucide-react"
import { projectRegistry } from "../../stores/ProjectRegistry"
import { createDiffNotesStore, type DiffNote } from "../../stores/DiffNotesStore"
import { createCommitDiffViewStore } from "../../stores/CommitDiffViewStore"
...
export const CommitDiffDialog = observer(function CommitDiffDialog({
  open,
  onOpenChange,
  workspacePath,
  commit,
  ...
  const diffStore = useMemo(
    () => (gitService ? createCommitDiffViewStore(workspacePath, commit, gitService) : null),
    [workspacePath, commit, gitService]
  )
  ...
  // NOTE: Include commit reference in the message
  useEffect(() => {
    notesStore.setOnSubmit((note) => {
      ...
      const message = `Comment on ${note.filePath} in commit ${commit.shortId} (${lineRef}):\n\`\`\`\n${note.lineContent}\n\`\`\`\n\n${note.comment}`
      const workspaceStore = projectRegistry.selectedWorkspaceStore
      workspaceStore?.sendMessage(message)
    })
  }, [notesStore, commit.shortId])
  ...
  const handleSubmitReview = useCallback(() => {
    // Format review message with commit reference
    const notes = notesStore.notes
    if (notes.length === 0) return

    const parts = notes.map((note: DiffNote) => {
      const lineRef =
        note.startLine === note.endLine
          ? `line ${note.startLine}`
          : `lines ${note.startLine}-${note.endLine}`
      return `### ${note.filePath} in commit ${commit.shortId} (${lineRef})\n\`\`\`\n${note.lineContent}\n\`\`\`\n${note.comment}`
    })

    const message = `## Code Review for commit ${commit.shortId}\n\n${parts.join("\n\n---\n\n")}`
    const workspaceStore = projectRegistry.selectedWorkspaceStore
    workspaceStore?.sendMessage(message)
    ...
    diffStore?.reset()
    onOpenChange(false)
  }, [notesStore, diffStore, onOpenChange, commit.shortId])
  ...
  const handleNoteClick = useCallback(
    (note: DiffNote) => {
      if (!diffStore) return
      const targetFile = diffStore.files.find((f) => f.path === note.filePath)
  ...
  const handleComment = useCallback(
    (
      ...
      const message = `Comment on ${_filePath} in commit ${commit.shortId} (${lineRef}):\n\`\`\`\n${lineContent}\n\`\`\`\n\n${comment}`
      const workspaceStore = projectRegistry.selectedWorkspaceStore
      workspaceStore?.sendMessage(message)
    },
    [commit.shortId]
  ...
  // Handle ESC when not in textarea
  ...
  }, [open, diffStore, handleSelectFile, workspacePath])
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-ovr-border-subtle px-4 py-3">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <GitCommit size={14} className="shrink-0 text-ovr-azure-400" />
              <span className="shrink-0 font-mono text-sm text-ovr-azure-400">
                {commit.shortId}
              </span>
              <span className="hidden text-xs text-ovr-text-dim md:inline">{commit.message}</span>
            </div>
          ...
          {/* Sub-header showing current file */}
          {diffStore.selectedFile.path && (
            <div className="flex items-center gap-2 border-b border-ovr-border-subtle bg-ovr-bg-app px-4 py-2">
          ...
          <div className="flex min-h-0 flex-1">
            ...
            {sidebarVisible && (
              <div className="w-56 shrink-0 overflow-y-auto border-r border-ovr-border-subtle bg-ovr-bg-panel">
                {diffStore.filesLoading && (
                  <div className="flex h-full items-center justify-center text-sm text-ovr-text-muted">
                    Loading files...
                  </div>
                )}
                {diffStore.filesError && (
                  <div className="p-3 text-sm text-ovr-bad">{diffStore.filesError}</div>
                )}
                {!diffStore.filesLoading &&
                  !diffStore.filesError &&
                  diffStore.files.length === 0 && (
                    <div className="flex h-full items-center justify-center text-sm text-ovr-text-muted">
                      No files changed
                    </div>
                  )}
                {!diffStore.filesLoading && diffStore.files.length > 0 && (
                  <div>
                    <button
                    ...
                    >
                      ...
                      <span className="flex-1">Files Changed</span>
                      <span className="text-ovr-text-dim">{diffStore.files.length}</span>
                    </button>
                    {!filesCollapsed &&
                      diffStore.files.map((file) => {
                        const style = STATUS_STYLES[file.status] ?? STATUS_STYLES["?"]
                        const isSelected = file.path === diffStore.selectedFile.path
                        return (
                          <button
          ...
          <AlertDialog.Description className="sr-only">
            Diff view for commit {commit.shortId}
